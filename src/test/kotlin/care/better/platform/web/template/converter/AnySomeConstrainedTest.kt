...
class AnySomeConstrainedTest : AbstractWebTemplateTest() {
    ...
    fun testAnyElementQuantity() {
        ...
        values["encounter/test_any/any_element/quantity_value|magnitude"] = 3

        val composition = getComposition(values)
        val quantity = validateAndGetElementValue(composition, DvQuantity::class.java)!!
        assertThat(quantity.magnitude).isEqualTo(3.0)
        assertThat(quantity.units).isEqualTo("kg")

        val formatted: Map<String, String?> = webTemplate.convertFormattedFromRawToFlat(composition, FromRawConversion.create())
        assertThat(formatted).contains(entry("encounter/test_any:0/any_element/quantity_value|magnitude", "3.0"))
        assertThat(formatted).contains(entry("encounter/test_any:0/any_element/quantity_value|unit", "kg"))
    }
    ...
    fun testAnyElementText() {
        ...
        val formatted: Map<String, String?> = webTemplate.convertFormattedFromRawToFlat(composition, FromRawConversion.create())
        assertThat(formatted).contains(entry("encounter/test_any:0/any_element/text_value", "Hello world!"))
    }
    ...
    fun testAnyElementCodedText() {
        ...
        val formatted: Map<String, String?> = webTemplate.convertFormattedFromRawToFlat(composition, FromRawConversion.create())
        assertThat(formatted).contains(entry("encounter/test_any:0/any_element/coded_text_value|value", "Hello world!"))
        assertThat(formatted).contains(entry("encounter/test_any:0/any_element/coded_text_value|code", "HW"))
        assertThat(formatted).contains(entry("encounter/test_any:0/any_element/coded_text_value|terminology", "mine"))
    }
